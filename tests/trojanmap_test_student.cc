...
TEST(TrojanMapTest, Autocomplete)
{
  ...
  EXPECT_EQ(names.empty(), true);

  // Test nonexistent prefix
  names = m.Autocomplete("ggg");
  EXPECT_EQ(names.empty(), true);
...
TEST(TrojanMapTest, FindPosition)
{
  ...
  // Test CAPS insensitive input
  position = m.GetPosition("chick-fil-a");
  EXPECT_EQ(position, gt1);
  ...
  // Test input with typo
  position = m.GetPosition("Chik-fil-A");
  EXPECT_EQ(position, gt4);
}
...
TEST(TrojanMapTest, CalculateEditDistance)
{
  ...
  EXPECT_EQ(m.CalculateEditDistance("hello", "world"), 4);

  // Test with one string empty
  EXPECT_EQ(m.CalculateEditDistance("", "nothing"), 7);

  // Test same string input
  EXPECT_EQ(m.CalculateEditDistance("awesome", "awesome"), 0);
}
...
TEST(TrojanMapTest, FindClosestName)
{
  ...
  // Test when no match
  EXPECT_EQ(m.FindClosestName("HelloWorld"), "");
}
...
TEST(TrojanMapTest, CalculateShortestPath_Dijkstra)
{
  ...
  path = m.CalculateShortestPath_Dijkstra("Hello", "World");
  EXPECT_EQ(path.empty(), true);

  // Test destination that is not possible
  path = m.CalculateShortestPath_Dijkstra("Ralphs", "DoesNotExist");
  EXPECT_EQ(path.empty(), true);
}
...
TEST(TrojanMapTest, CalculateShortestPath_Bellman_Ford)
{
  ...
  // Test locations that don't exist
  path = m.CalculateShortestPath_Bellman_Ford("Hello", "World");
  EXPECT_EQ(path.empty(), true);

  // Test destination that is not possible
  path = m.CalculateShortestPath_Bellman_Ford("Ralphs", "DoesNotExist");
  EXPECT_EQ(path.empty(), true);
}
...
TEST(TrojanMapTest, FindNearby)
{
  ...
  // test 2
  auto result2 = m.FindNearby("beauty", "Dulce", 15, 5);
  std::vector<std::string> ans2{"6047204780", "9591449459"};
  EXPECT_EQ(result2, ans2);

  // test 3
  auto result3 = m.FindNearby("fuel", "Dulce", 5, 5);
  std::vector<std::string> ans3{"591042107", "9462723984", "9462770200", "1759017535", "3390316852"};
...
TEST(TrojanMapTest, Queries)
{
  ...
  // test 2
  std::vector<std::pair<double, std::vector<std::string>>> input2{{0.05, {"Parkside Dining Hall", "Subway"}},
                                                                  {0.05, {"Target", "Ralphs"}},
                                                                  {0.02, {"Bank of America", "Dulce"}}};
  auto actual2 = m.Queries(input2);
  std::vector<bool> expected2{false, true, false};
  EXPECT_EQ(expected2, actual2);

  // test 3
  std::vector<std::pair<double, std::vector<std::string>>> input3{{0.8, {"Dulce", "Subway"}},
                                                                  {0.5, {"Subway", "901 Bar & Grill"}},
                                                                  {1.0, {"901 Bar & Grill", "Honda"}}};
  auto actual3 = m.Queries(input3);
  std::vector<bool> expected3{true, true, true};
